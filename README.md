...
1. Create a free database: [Turso via the Vercel Marketplace](https://vercel.com/marketplace/tursocloud)
   (or directly at [turso.tech](https://turso.tech) / the `turso` CLI). You need two values: a
   URL and a separate auth token (`turso db tokens create <name>`). **Use the `https://`
   scheme for `DATABASE_URL`, not `libsql://`** — Turso's dashboard/CLI usually hand you the
   `libsql://` form, which `libsql-client` maps to a WebSocket connection; that WebSocket
   upgrade fails on Vercel's serverless networking (`400 Invalid response status` during the
   handshake). Swapping the scheme to `https://<db>-<org>.turso.io` routes over plain HTTP
   instead, which works and is also the better fit for one-connection-per-invocation anyway.
...
`display_messages`, `strategy_config`) is created automatically on first real request that
needs it (lazily, inside `app/turso.py`'s `get_client()` — not at cold-start import time,
since that would make a not-yet-valid `DATABASE_URL` crash the whole process before any
request arrives). No separate migration step. Vercel's Python framework detection finds the
`FastAPI` instance named `app` in `app/main.py` automatically — `vercel.json` only sets
`maxDuration: 60` on that entrypoint, since an agent turn is a live LLM call, not instant; no
`vercel.json` rewrites or `api/` directory are needed (an earlier version of this doc had
both — they actively conflicted with the zero-config routing and produced 404s on every
route).
